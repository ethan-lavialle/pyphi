...
> Tip: remove only the functions you are certain you do not need. Some helpers (for example `z2n`) are tightly coupled with other routines; if you decide later to re-enable a broader feature set, keep a copy of the full `pyphi.py` for reference.
